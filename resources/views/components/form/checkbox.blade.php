...
<label class="inline-flex items-center gap-2.5 cursor-pointer group">
	<span class="relative flex items-center justify-center size-3.5">
		<input
			...
			@checked($checked)
			{{ $attributes->merge([
				'class' => 'peer appearance-none size-3.5 border border-neutral-300 bg-white checked:bg-neutral-900 checked:border-neutral-900 focus:ring-0 focus:ring-offset-0 cursor-pointer transition-colors'
			]) }}
		/>
		<svg
	...
	<span class="text-sm text-neutral-600 group-hover:text-neutral-900 transition-colors select-none">{{ $slot }}</span>
</label>
